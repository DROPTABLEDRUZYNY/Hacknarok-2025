...
const Counter = ({ initialValue, description }: { initialValue: number; description: string }) => {
  ...
  return (
    ...
    >
      <motion.span className="text-7xl font-bold bg-gradient-to-r from-purple-600 via-pink-500 to-blue-500 inline-block text-transparent bg-clip-text">
        {rounded}
      </motion.span>
      <span className="font-extralightmt-2 text-center w-[150px] text-sm">{description}</span>
    </motion.div>
  );
};
